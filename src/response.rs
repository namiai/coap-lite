use crate::{
    packet::Packet, CoapMessageExt, CoapRequest, MessageClass, MessageType,
    ResponseType,
};
...
impl<T: Packet> CoapResponse<T> {
    ...
    pub fn from_request(
        request: &CoapRequest<T>,
        response_type: ResponseType,
    ) -> Option<CoapResponse<T>> {
...
mod test {
    ...
    fn test_new_response_from_request_valid() {
        for mtyp in vec![MessageType::Confirmable, MessageType::NonConfirmable]
        {
            ...
            packet.set_code_from_message_class(MessageClass::Request(
                RequestType::Get,
            ));
